...
private:
    ...
    public:
        void Event(const uint32_t id, const string &eventName, const string &parameters) override
        {
            _parent.onMediaEngineEvent(id, eventName, parameters);
        }
    ...
    //TODO: needed? - always creates separate process
    ...
    uint32_t setPosition(const JsonObject& parameters, JsonObject& response);
    uint32_t stop(const JsonObject& parameters, JsonObject& response);

    void onMediaEngineEvent(const uint32_t id, const string &eventName, const string &parameters);
